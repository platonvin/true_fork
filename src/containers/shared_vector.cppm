...
// because there is only one allocation and only one size var. Used by ECManager
...
export module shared_vector;

export template <typename... Types>
class shared_vector {
  private:
    // Pointers to memory blocks of each stored type.
    tuple<Types *...> data_pointers = {};
    ...
    // Raw allocated memory pointer for all elements.
    uint8_t *allocated_mem_ptr = nullptr;

    // Helper to calculate the offset of each type in our contiguous memory block.
    ...
    template <size_t Index>
    constexpr void copy_data_recursive(uint8_t *new_memory, size_t old_size,
  ...
  public:
    ...
    // Get the current size (length of each sub-vector, they are all the same).
    constexpr inline size_t size() const {
        return size_;
    }

    // Get the current capacity (capacity of each sub-vector, they are all the same).
