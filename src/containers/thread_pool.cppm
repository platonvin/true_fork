...
// simple, but not-so energy efficient thread pool
// works by busy-waiting until signaled and sent work
...
export class thread_pool {
  ...
  private:
    ...
    thread_pool() : threads_active(0), should_stop(false) {
        ...
        thread_flags = new std::atomic<bool>[num_threads];
        ...
        for (int i = 0; i < num_threads; i++) {
            threads[i] = std::thread([this, i]() { this->worker_thread_fun(i); });
        }
    }
    ...
    void worker_thread_fun(int thread_id) {
        ...
        while (true) {
            ...
            if (should_stop)
                break; // If stop flag is set, exit the loop
            ...
            if (task) {
                ...
                // After task completion, decrement the counter. When it reaches 0, it means that
                // all threads are done with that task
                threads_active -= 1;
            }
        }
    }
    ...
    alignas(std::hardware_destructive_interference_size)
        ...
        std::atomic<bool> *thread_flags;
    alignas(std::hardware_destructive_interference_size) std::atomic<int> threads_active =
        0; // Counter to track the number of remaining tasks
    alignas(std::hardware_destructive_interference_size) std::atomic<bool> should_stop =
        false; // Flag to signal stopping the pool
};