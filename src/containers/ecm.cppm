 ...
 *
 * @tparam ...Components The component types (structs/classes) managed by the ECS. They HAVE to be
 * different. If you need the same component multiple times, use typedef. This is required for
 * components to be identifiable in compile-time by their type. Typedef just gives them unique
 * "name"
 * @example typedef float hp; typedef float damage; ECManager<hp, damage> ecm{};
 */
export using EntityID = long long; ///< Type alias for entity identifiers.
export using InternalIndex = int; ///< Type alias for entity identifiers.
...
struct ECManager {
    ...
    constexpr inline void destroyEntity(EntityID id) {
        ...
        ASSERT(it != idToInternalIndex.end(),
            "Entity with ID " + std::to_string(id) + " not found on Destroy.");
    ...
    constexpr inline Component& getEntityComponent(EntityID id) {
        auto it = idToInternalIndex.find(id);
        ASSERT(it != idToInternalIndex.end(),
            "Entity with ID " + std::to_string(id) + " not found on Get Component.");
        InternalIndex index = it->second;
    ...
    constexpr inline std::tuple<Components&...> getEntityComponents(EntityID id) {
        auto it = idToInternalIndex.find(id);
        ASSERT(it != idToInternalIndex.end(),
            "Entity with ID " + std::to_string(id) + " not found on Get Component_S.");
        InternalIndex index = it->second;
        return getEntityComponentsInternal(index);
    }

    constexpr inline std::tuple<Components&...> getEntityComponentsInternal(
...
#pragma simd
    ...
    constexpr inline void loadAndInvoke(Func&& func, InternalIndex id) {
        ...
        if constexpr (std::is_same_v<std::tuple_element_t<0, ArgNoRefTuple>, InternalIndex>) {
            auto components_without_ii =
                loadComponentsFromTupleInternal<tuple_tail_t<ArgNoRefTuple>>(id);
    ...
    //         auto components_without_ii =
    //         loadComponentsFromTupleInternal<tuple_tail_t<ArgNoRefTuple>>(id); auto components =
    //         std::tuple_cat(std::make_tuple(id), components_without_ii);
    ...
    constexpr inline auto loadComponentsFromTupleImplInternal(
        InternalIndex id, std::index_sequence<I...>) {
        return std::forward_as_tuple(
            getEntityComponentInternal<std::tuple_element_t<I, Tuple>>(id)...);
    }

    template <typename Tuple>
    constexpr inline auto loadComponentsFromTupleInternal(InternalIndex id) {
        return loadComponentsFromTupleImplInternal<Tuple>(
            id, std::make_index_sequence<std::tuple_size_v<Tuple>>{});
    }
    ...
    void update() {
        ...
        for (InternalIndex readIndex = 0; readIndex < old_size; readIndex++) {
            ...
            if (is_read_alive) {
                componentArrays.template get<bool>()[writeIndex] =
                    componentArrays.template get<bool>()[readIndex];
                componentArrays.template get<EntityID>()[writeIndex] =
                    componentArrays.template get<EntityID>()[readIndex];
                writeIndex++;
    ...
    //         componentArrays.template get<bool>()[to] = componentArrays.template
    //         get<bool>()[from];
    //     }
    // }

    // this is like vectors, one per component
    shared_vector<bool, ///< Aka cached delete. Flags indicating whether an entity is alive
    ...
    // ankerl::unordered_dense::segmented_map<EntityID, InternalIndex> idToInternalIndex;  ///<
    // Mapping of entity IDs to internal indices.
    std::unordered_map<EntityID, InternalIndex>
        idToInternalIndex; ///< Mapping of entity IDs to internal indices.
    // absl::flat_hash_map<EntityID, InternalIndex> idToInternalIndex;  ///< Mapping of entity IDs
    // to internal indices. gtl::btree_map<EntityID, InternalIndex> idToInternalIndex;  ///< Mapping
    // of entity IDs to internal indices.
 ...
 * The class supports a variadic number of component types and a variadic number of update
 * functions.
 *
 ...
 * @tparam UpdateFuncs Systems (functions) that operate on this ECS. They are called in a given
 * order every frame (update).
 *
 ...
 */
export template <typename ECManager_type, typename... UpdateFuncs>
struct ECSystem {
  public:
     ...
     */
    constexpr inline ECSystem(UpdateFuncs... funcs) : functions(std::make_tuple(funcs...)) {}
     ...
     * @brief Retrieves a specific component from a given entity.
     *
     * @tparam Component The type (no reference - its just syntax sugar overload) of the component
     * to retrieve.
     * @param id The ID of the entity from which to retrieve the component.
     * @return Component& reference to the requested component.
     */
     ...
     * @return std::tuple<Components&...> references to all components associated for the specified
     * entity.
     */
    ...
    constexpr inline void update(ECManager_type& ecm) {
        ...
        std::apply(
            [&, this](UpdateFuncs&... funcs) { (ecm.forEachEntityWith(funcs), ...); }, functions);
    }
