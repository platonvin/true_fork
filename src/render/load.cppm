...
void LumInternal::LumInternalRenderer::updateBlockPalette(BlockWithMesh* blockPalette) {
    // block palette is in u8, but for easy copying we convert it to u16 cause block palette is
    // R16_UNIT
...
void LumInternal::LumInternalRenderer::load_mesh(
    InternalMeshModel* mesh,
    const char* vox_file,
    bool _make_vertices,
    bool extrude_palette,
    Material* mat_palette
) {
    ATRACE();
    auto buffer = readFileBuffer(vox_file);
    ATRACE();
    ...
    load_mesh(
        mesh,
        (Voxel*)scene->models[0]->voxel_data,
    ...
    for (int x = 0; x < scene->models[0]->size_x; x++) {
        for (int y = 0; y < scene->models[0]->size_y; y++) {
            for (int z = 0; z < scene->models[0]->size_z; z++) {
                (block)->voxels[x][y][z] =
                    (u16)scene->models[0]
                        ->voxel_data[x + y * scene->models[0]->size_x + z * scene->models[0]->size_x * scene->models[0]->size_y];
            }
        }
    }
    TRACE()
...
void LumInternal::LumInternalRenderer::free_mesh(InternalMeshModel* mesh) {
    ...
    for (int i = 0; i < lumal.settings.fif; i++) {
        ...
        // vmaDestroyBuffer(render.VMAllocator, mesh->triangles.vertexes[i].buffer,
        // mesh->triangles.vertexes[i].alloc);
        // }
...
bool operator==(const LumInternal::PackedVoxelVertex& one, const LumInternal::PackedVoxelVertex& other) {
    return (one.pos.x == other.pos.x) && (one.pos.y == other.pos.y) && (one.pos.z == other.pos.z);
}
...
LumInternal::PackedVoxelQuad pack_quad(const std::array<LumInternal::PackedVoxelVertex, 6> vertices, uvec3 norm) {
    ...
    diff = high - low;
    assert(glm::any(greaterThan(diff, {0, 0, 0})));
...
void LumInternal::LumInternalRenderer::make_vertices(InternalMeshModel* mesh, Voxel* Voxels, int x_size, int y_size, int z_size) {
    ...
    for (u32 i = 0; i < ogt_mesh->vertex_count; i++) {
        ...
        assert(!glm::any(greaterThanEqual(ogt_mesh->vertices[i].pos, uvec3(255))));
    ...
    for (u32 i = 0; i < ogt_mesh->index_count; ++i) {
        ...
        if (target_vector)
            target_vector->push_back(index);
        else
            crash("Unrecognized normal");
    }
    // TRACE();
    assert(circ_verts.size() == ogt_mesh->vertex_count);
    // mesh->triangles.vertexes = render.createElemBuffer<PackedVoxelCircuit> (circ_verts.data(),
    // circ_verts.size(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
    // TRACE();
