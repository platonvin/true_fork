...
// #define transparent
// (*((Lum::OpaqueRendererMembers*)(&this->promised_opaque_members)))
...
Lum::Renderer::Renderer(
    ...
    liquid_que(),
    volumetric_que() {
    // this calls constructors on memory that is visible to Renderer as just
    // bytes, but in fact represents Lum::OpaqueRendererMembers
...
void Lum::Renderer::init(Lum::Settings lum_settings) noexcept {
    ...
    LumInternal::LumSettings internal_settings = {};
    ...
    //  internal_settings.debug = false; //Validation Layers. Use them while
    //  developing or be tricked into thinking that your code is correct
    //  internal_settings.timestampCount = 48;
    internal_settings.profile = true; // monitors perfomance via timestamps. You can place one with
    // PLACE_TIMESTAMP() macro
    // currently fif has bug in it, do not change for now
    internal_settings.fif = LumInternal::MAX_FRAMES_IN_FLIGHT; // Frames In Flight. If 1, then record
    // cmdbuff and submit it. If multiple,
    // cpu will (might) be ahead of gpu by
    // FIF-1, which makes GPU wait less
    ...
    // for assumeEXT, currently disabled and will be brought back with new
    // shader system VkPhysicalDeviceShaderExpectAssumeFeaturesKHR
    //     expect;
    //     expect.sType =
    ...
    internal_settings.deviceExtensions.push_back(VK_KHR_PUSH_DESCRIPTOR_EXTENSION_NAME); // simplifies renderer for
    // negative cost lol
...
void Lum::Renderer::loadBlock(MeshBlock block, LumInternal::BlockVoxels* block_data) noexcept {
    memcpy((void*)&opaque_members->block_palette[block], block_data, sizeof(LumInternal::BlockVoxels));
...
Lum::MeshModel Lum::Renderer::loadModel(const std::string& file, bool try_extract_palette) noexcept {
    ...
    opaque_members->render.load_mesh(
        new_mesh,
        file.c_str(),
        true,
        /*extract palette if no found = */ try_extract_palette,
        opaque_members->mat_palette.data()
    );
    ATRACE();
    return new_mesh;
}
...
Lum::MeshFoliage Lum::Renderer::loadFoliage(const std::string& vertex_shader_file, int vertices_per_foliage, int density) noexcept {
    LumInternal::InternalMeshFoliage* new_foliage = opaque_members->mesh_foliage_storage.allocate();
    // this is not creating pipe(line). Only queuing shader file for later
    // pipe(line) i just hope C++ is not fucked up and lifetime of string is
    // whole main
...
Lum::MeshVolumetric Lum::Renderer::loadVolumetric(float max_density, float density_deviation, const u8vec3& color) noexcept {
    LumInternal::InternalMeshVolumetric* new_volumetric = opaque_members->mesh_volumetric_storage.allocate();
...
Lum::MeshLiquid Lum::Renderer::loadLiquid(LumInternal::MatID_t main_mat, LumInternal::MatID_t foam_mat) noexcept {
    LumInternal::InternalMeshLiquid* new_liquid = opaque_members->mesh_liquid_storage.allocate();
    ...
    for (int xx = 0; xx < 2; xx++) {
        for (int yy = 0; yy < 2; yy++) {
            for (int zz = 0; zz < 2; zz++) {
                ...
                if ((clip.x >= -1.0) && (clip.y >= -1.0) && (clip.z >= -1.0) && (clip.x <= +1.0) && (clip.y <= +1.0) && (clip.z <= +1.0)) {
                    return true;
                }
            }
        }
...
static bool is_model_visible(const Lum::MeshModel& model, const Lum::MeshTransform& trans, const glm::mat4& viewProjMatrix) {
    ...
    for (let corner : transformedCorners) {
        ...
        if ((clip.x >= -1.0f) && (clip.y >= -1.0f) && (clip.z >= -1.0f) && (clip.x <= +1.0f) && (clip.y <= +1.0f) && (clip.z <= +1.0f)) {
            return true;
        }
    }
    return false;
        ...
        for (int yy = 0; yy < opaque_members->settings.world_size.y; yy++) {
            for (int xx = 0; xx < opaque_members->settings.world_size.x; xx++) {
                ...
                if (block_id != 0) {
                    ...
                    // vec3 clip_coords = (render.camera.cameraTransform *
                    // vec4(brr.pos,1));
                ...
                TRACE()
                assert(
                    block_id <= opaque_members->render.static_block_palette_size && "there is block in the world that is outside of the palette"
                );
            }
        }
    }
...
static bool isModelVisible(const Lum::MeshModel& model, const Lum::MeshTransform& trans, const glm::dvec3& worldSize) {
    ...
    for (const auto& corner : transformedCorners) {
        if (corner.x >= 0.0 && corner.y >= 0.0 && corner.z >= 0.0 && corner.x <= worldSize.x && corner.y <= worldSize.y
            && corner.z <= worldSize.z) {
            return true;
        }
    }
    return false;
...
// nevery knew std::sort is allowed to segfault on wrong __comp. Why everything
// has exceptions except for things that need it?
...
void calculateAndSortByCamDist(Container& render_queue, const glm::dmat4& cameraTransform) {
    ...
    for (let rrequest : render_queue) {
        ...
        rrequest.cam_dist = -clip_coords.z; // set cam_dist to negative z cause i
        // never get it right (depth sorting)
    }

    // sort render requests by cam_dist in descending order
    auto sortByCamDist = [](const RenderRequestType& lhs, const RenderRequestType& rhs) { return lhs.cam_dist > rhs.cam_dist; };
...
void Lum::Renderer::prepareFrame() noexcept {
    glfw::pollEvents();
    should_close |= glfw::windowShouldClose(opaque_members->render.lumal.window.pointer);
...
void Lum::Renderer::submitFrame() noexcept {
    ...
    TRACE()
    // opaque_members->render.recalculate_df(); // currently unused. per-voxel
    // Distance Field opaque_members->render.recalculate_bit(); // currently
    // unused. Bitpacking, like (block==Air) ? 0 : 1
    TRACE()
    ...
    // otherwise (if after) there is more (visible!) inconsistency which might
    // seem like its perfomance problems
    updateTime();
...
void Lum::Renderer::cleanup() noexcept {
    ...
    for (int i = 0; i < opaque_members->mesh_models_storage.total_size(); i++) {
        ...
        if (is_allocated) {
            ...
            // no free cause not needed and will actually make referenced memory
            // illegal mesh_models_storage.free(imm_ptr);
        }
    }
    waitIdle();
    TRACE();
