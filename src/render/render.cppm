...
#include "lumal/macros.hpp"

using std::vector;
using namespace glm;
using namespace Lumal;

export module lum:render;

#define POS_NORM_BIT_MASK 0b10000000

export namespace LumInternal {
...
typedef struct InternalMeshModel {
    ...
    // 3d image of voxels in this mesh, used to represent mesh to per-frame world voxel
    // representation
...
struct LumInternalRenderer {
  ...
  public:
    ...
    void load_mesh(
        InternalMeshModel* mesh,
        const char* vox_file,
        bool _make_vertices = true,
        bool extrude_palette = true,
        Material* mat_palette = nullptr
    );
    // sets voxels and size. By default uses first .vox palette as main palette
    ...
    void end_blockify();
    void blockify_custom(void* ptr); // just in case you have custom blockify algorithm. If using
    // this, no startBlockify needed
    void exec_copies();
    void start_map();
    ...
    void lightmap_block(InternalMeshModel* block_mesh, int block_id, ivec3 shift);
    void lightmap_block_face(i8vec3 normal, IndexedVertices& buff, int block_id);
    void lightmap_start_models();
    void lightmap_model(InternalMeshModel* mesh, MeshTransform* model_trans);
    void lightmap_model_face(vec3 normal, IndexedVertices& buff);
    void end_lightmap(); // ends somewhere after raygen but operates on separate cmd buf
    // void start_lightmap();
    void start_raygen();
    void raygen_start_blocks();
    void raygen_block(InternalMeshModel* block_mesh, int block_id, ivec3 shift);
    void draw_block_face(i8vec3 normal, IndexedVertices& buff, int block_id);
    void raygen_start_models();
    void raygen_model(InternalMeshModel* mesh, MeshTransform* model_trans);
    void draw_model_face(vec3 normal, IndexedVertices& buff);
    ...
    vector<Lumal::Buffer> createElemBuffers(Vertex_T* vertices, u32 count, u32 buffer_usage = 0);
    template<class Vertex_T>
    vector<Lumal::Buffer> createElemBuffers(vector<Vertex_T> vertices, u32 buffer_usage = 0);
    ...
    ring<Lumal::Image> perlinNoise2d; // full-world grass shift (~direction) texture sampled in
    // grass
  ...
  private:
    ...
    const VkFormat DEPTH_FORMAT_SPARE = VK_FORMAT_D32_SFLOAT_S8_UINT; // TODO somehow faster than VK_FORMAT_D24_UNORM_S8_UINT on
    // low-end
...
class MyRenderInterface {
  public:
    ...
    // Called by RmlUi when it wants to render geometry that the application does not wish to
    // optimise void RenderGeometry (Rml::Vertex* vertices,
    //                      int num_vertices,
    //                      int* indices,
    //                      int num_indices,
    ...
    // // Called by RmlUi when it wants to compile geometry it believes will be static for the
    // forseeable future. Rml::CompiledGeometryHandle CompileGeometry (Rml::Vertex* vertices, int
    // num_vertices, int* indices, int num_indices, Rml::TextureHandle texture); //override;

    // // Called by RmlUi when it wants to render application-compiled geometry.
    // void RenderCompiledGeometry (Rml::CompiledGeometryHandle geometry, const Rml::Vector2f&
    ...
    // sequence of pixels. bool GenerateTexture (Rml::TextureHandle& texture_handle,
    ...
    // // If no transform applies to the current element, nullptr is submitted. Then it expects the
    // renderer to use
...
#define PLACE_TIMESTAMP_OUTSIDE(commandBuffer) \
    do { \
        if (lumal.settings.profile) { \
            ...
            vkCmdWriteTimestamp( \
                ...
                lumal.queryPoolTimestamps.current(), \
                lumal.currentTimestamp++ \
            ); \
        } \
    } while (0)
...
void LumInternal::LumInternalRenderer::init(LumSettings settings) {
    ...
    DEPTH_FORMAT = lumal.findSupportedFormat(
...
void LumInternal::LumInternalRenderer::createImages() {
    lumal.createBufferStorages(
        &stagingWorld,
        VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        world_size.x * world_size.y * world_size.z * sizeof(BlockID_t),
        true
    );
    lumal.createImageStorages(
        &world,
        VK_IMAGE_TYPE_3D,
        VK_FORMAT_R16_SINT,
    ...
    lumal.createImageStorages(
        ...
        VK_IMAGE_ASPECT_COLOR_BIT,
        {6, 256, 1}
    ); // TODO: dynamic, text formats, pack Material into smth other than 6 floats :)
    lumal.createImageStorages(
        &grassState,
        VK_IMAGE_TYPE_2D,
        ...
        0,
        VK_IMAGE_ASPECT_COLOR_BIT,
        {world_size.x * 2, world_size.y * 2, 1}
    ); // for quality
    lumal.createImageStorages(
        &waterState,
        VK_IMAGE_TYPE_2D,
    ...
    lumal.createBufferStorages(
        ...
        maxParticleCount * sizeof(Particle),
        true
    );
    lumal.createBufferStorages(
        &uniform,
        ...
        220,
        false
    ); // no way i write it with sizeof's
    lumal.createBufferStorages(&lightUniform, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, sizeof(mat4), false);
    lumal.createBufferStorages(
        &aoLutUniform,
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        sizeof(AoLut) * 8,
    ...
    lumal.createBufferStorages(
        ...
        sizeof(i8vec4) * world_size.x * world_size.y * world_size.z,
        false
    ); // TODO test extra mem
    lumal.createBufferStorages(
        &stagingRadianceUpdates,
        VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        sizeof(ivec4) * world_size.x * world_size.y * world_size.z,
        true
    ); // TODO test extra mem
...
void LumInternal::LumInternalRenderer::setupDescriptors() {
    ...
    lumal.deferDescriptorSetup(
        &lightmapBlocksPipe.setLayout,
        &lightmapBlocksPipe.sets,
        {
    ...
    lumal.deferDescriptorSetup(
        &lightmapModelsPipe.setLayout,
        &lightmapModelsPipe.sets,
        {
            {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, RDP::CURRENT, (&lightUniform), {/*empty*/}, NO_SAMPLER, NO_LAYOUT},
        },
        VK_SHADER_STAGE_VERTEX_BIT
    ...
    lumal.deferDescriptorSetup(
        ...
        &diffusePipe.sets,
        {
            ...
            // {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, RDP::FIRST, {/*empty*/}, {lightmap},
            // linearSampler, VK_IMAGE_LAYOUT_GENERAL},
        },
    ...
    TRACE();
    lumal.deferDescriptorSetup(
        &fillStencilGlossyPipe.setLayout,
        &fillStencilGlossyPipe.sets,
        {
            {VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, RDP::FIRST, {/*empty*/}, &highresMatNorm, NO_SAMPLER, VK_IMAGE_LAYOUT_GENERAL},
            {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, RDP::CURRENT, {/*empty*/}, (&materialPalette), nearestSampler, VK_IMAGE_LAYOUT_GENERAL},
        },
    ...
    TRACE();
    lumal.deferDescriptorSetup(
        &fillStencilSmokePipe.setLayout,
        &fillStencilSmokePipe.sets,
        {
            {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, RDP::CURRENT, (&uniform), {/*empty*/}, NO_SAMPLER, NO_LAYOUT},
        },
        VK_SHADER_STAGE_VERTEX_BIT
    ...
    // VK_DESCRIPTOR_SET_LAYOUT_CREATE_PUSH_DESCRIPTOR_BIT_KHR);
    lumal.deferDescriptorSetup(
        &raygenBlocksPipe.setLayout,
        &raygenBlocksPipe.sets,
        {
            {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, RDP::CURRENT, (&uniform), {/*empty*/}, NO_SAMPLER, NO_LAYOUT},
    ...
    // 0);
    lumal.deferDescriptorSetup(
        &raygenModelsPipe.setLayout,
        &raygenModelsPipe.sets,
        {
            {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, RDP::CURRENT, (&uniform), {/*empty*/}, NO_SAMPLER, NO_LAYOUT},
        },
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT
    ...
    TRACE();
    lumal.deferDescriptorSetup(
        &raygenParticlesPipe.setLayout,
        &raygenParticlesPipe.sets,
        {
            {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, RDP::CURRENT, (&uniform), {/*empty*/}, NO_SAMPLER, NO_LAYOUT},
    ...
    lumal.deferDescriptorSetup(
        &updateGrassPipe.setLayout,
        &updateGrassPipe.sets,
        {
    ...
    lumal.deferDescriptorSetup(
        &updateWaterPipe.setLayout,
        &updateWaterPipe.sets,
        {
    ...
    //     {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, RDP::CURRENT, (&uniform), {/*empty*/}, NO_SAMPLER,
    //     NO_LAYOUT}, {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, RDP::FIRST, {}, &grassState,
    //     linearSampler, VK_IMAGE_LAYOUT_GENERAL},
    // }, VK_SHADER_STAGE_VERTEX_BIT);
    lumal.deferDescriptorSetup(
        &raygenWaterPipe.setLayout,
        &raygenWaterPipe.sets,
        {
            {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, RDP::CURRENT, (&uniform), {/*empty*/}, NO_SAMPLER, NO_LAYOUT},
    ...
    lumal.deferDescriptorSetup(
        &genPerlin2dPipe.setLayout,
        &genPerlin2dPipe.sets,
        {
    ...
    lumal.deferDescriptorSetup(
        &genPerlin3dPipe.setLayout,
        &genPerlin3dPipe.sets,
        {
    ...
    // {VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, RDP::FIRST, {}, {originBlockPalette}, NO_SAMPLER,
    // VK_IMAGE_LAYOUT_GENERAL}, {VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, RDP::FIRST, {}, {distancePalette
    // }, NO_SAMPLER, VK_IMAGE_LAYOUT_GENERAL},
    // }, VK_SHADER_STAGE_COMPUTE_BIT);
    ...
    // {VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, RDP::FIRST, {}, {originBlockPalette}, NO_SAMPLER,
    // VK_IMAGE_LAYOUT_GENERAL}, {VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, RDP::FIRST, {}, {bitPalette},
    // NO_SAMPLER, VK_IMAGE_LAYOUT_GENERAL},
    // }, VK_SHADER_STAGE_COMPUTE_BIT);
    TRACE();
...
void LumInternal::LumInternalRenderer::createFoliagePipilines() {
    for (auto* fol : registered_foliage) {
        ...
        lumal.createRasterPipeline(
            ...
            lumal.swapChainExtent,
            {Lumal::NO_BLEND},
            sizeof(vec4) + sizeof(int) * 2 + sizeof(int) * 2,
            Lumal::DEPTH_TEST_READ_BIT | Lumal::DEPTH_TEST_WRITE_BIT,
            VK_COMPARE_OP_LESS,
            VK_CULL_MODE_NONE,
            Lumal::NO_DISCARD,
            Lumal::NO_STENCIL
...
void LumInternal::LumInternalRenderer::createPipilines() {
    ...
    // if you cannot guess what things mean by just looking at them maybe read old (0.0.3) release
    // src
    TRACE();
    ...
    //     render.swapChainExtent, {Lumal::NO_BLEND}, sizeof (vec4) + sizeof (int) * 2 + sizeof
    //     (int) * 2, Lumal::DEPTH_TEST_READ_BIT|Lumal::DEPTH_TEST_WRITE_BIT, VK_COMPARE_OP_LESS,
    //     VK_CULL_MODE_NONE, Lumal::NO_DISCARD, Lumal::NO_STENCIL);
    // TRACE();
    ...
    lumal.createRasterPipeline(
        ...
        lumal.swapChainExtent,
        {Lumal::NO_BLEND},
        sizeof(ivec4) + sizeof(vec4) * 4 + sizeof(mat4),
        Lumal::DEPTH_TEST_NONE_BIT,
        VK_COMPARE_OP_LESS,
        VK_CULL_MODE_NONE,
        Lumal::NO_DISCARD,
        Lumal::NO_STENCIL
    ...
    lumal.createRasterPipeline(
        ...
        VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
        lumal.swapChainExtent,
        {Lumal::BLEND_REPLACE_IF_GREATER, Lumal::BLEND_REPLACE_IF_LESS},
        sizeof(vec3) + sizeof(int) + sizeof(vec4),
        Lumal::DEPTH_TEST_READ_BIT,
        VK_COMPARE_OP_LESS,
        VK_CULL_MODE_NONE,
        Lumal::NO_DISCARD,
        {
    ...
    TRACE();
    lumal.createComputePipeline(
        &radiancePipe,
        0,
        find_shader("radiance.comp.spv").c_str(),
        sizeof(int) * 4,
...
VkResult LumInternal::LumInternalRenderer::createSwapchainDependent() {
    ...
    // vkResetCommandPool(render.device, render.commandPool,
    ...
    lumal.createRenderPass(
        {
            ...
            {&farDepth,
             ...
             Lumal::DontCare,
             Lumal::DontCare,
             {.color = {.float32 = {-1000.0, -1000.0, -1000.0, -1000.0}}}},
            {&nearDepth,
             Lumal::Clear,
             Lumal::DontCare,
             Lumal::DontCare,
...
void LumInternal::LumInternalRenderer::createSwapchainDependentImages() {
    ...
    lumal.createImageStorages(
        ...
        VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT
            | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
        VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE,
        VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT,
        VK_IMAGE_ASPECT_COLOR_BIT,
    ...
    // render.transitionImageLayoutSingletime(&farDepth[0],
...
void LumInternal::Camera::updateCamera() {
    ...
    dmat4 projection =
        glm::ortho(-viewSize.x / 2.0, viewSize.x / 2.0, viewSize.y / 2.0, -viewSize.y / 2.0, -0.0, +2000.0); // => *(2000.0/2) for decoding
...
void LumInternal::LumInternalRenderer::update_light_transform() {
    ...
    dmat4 projection = glm::ortho(
        -view_width_in_voxels / 2.0,
        +view_width_in_voxels / 2.0,
        +view_height_in_voxels / 2.0,
        -view_height_in_voxels / 2.0,
        ...
        for (int yy = border.min.y; yy <= border.max.y; yy++) {
            for (int xx = border.min.x; xx <= border.max.x; xx++) {
                ...
                if (current_block < static_block_palette_size) { // static
                    ...
                    // do image copy on for non-zero-src blocks. Other things still done for every
                    // allocated block because zeroing is fast
                    if (current_block != 0) {
...
void set_blocks(const ivec3& coord, table3d<bool>& set, const table3d<LumInternal::BlockID_t>& current_world) {
...
void push_radiance_updates(const ivec3& size, table3d<bool>& set, std::vector<i8vec4>& radianceUpdates) {
...
void LumInternal::LumInternalRenderer::update_radiance() {
    ...
    /**/ vkCmdBindDescriptorSets(
        ...
        radiancePipe.lineLayout,
        0,
        1,
        &radiancePipe.sets.current(),
        0,
        0
    );
    ...
    struct rtpc {
        int time, iters, size, shift;
    } pushconstant = {lumal.iFrame, 0, magicSize, lumal.iFrame % magicSize};
...
void LumInternal::LumInternalRenderer::shift_radiance(ivec3 radiance_shift) {
    ...
    if (false || abs(cam_shift.x) >= world_size.x || abs(cam_shift.x) >= world_size.x || abs(cam_shift.x) >= world_size.x)
        return; // then its pointless (zero-volume intersection). We can set it to zero os some
    // pre-computed value in future, tho
    ...
    // where to copy from
    ivec3 self_src_offset =
        ivec3(process_axis(cam_shift.x, world_size.x).x, process_axis(cam_shift.y, world_size.y).x, process_axis(cam_shift.z, world_size.z).x);
    // where the intersection should end up (in the same image)
    ivec3 self_dst_offset =
        ivec3(process_axis(cam_shift.x, world_size.x).y, process_axis(cam_shift.y, world_size.y).y, process_axis(cam_shift.z, world_size.z).y);
    ...
    vkCmdCopyImage(
        commandBuffer.commandBuffer,
        radianceCache.current().image,
        VK_IMAGE_LAYOUT_GENERAL,
        radianceCache.previous().image,
        VK_IMAGE_LAYOUT_GENERAL,
        1,
        &copyRegion
    );
...
void LumInternal::LumInternalRenderer::exec_copies() {
    ...
    commandBuffer.cmdExplicitTransLayoutBarrier(
        VK_IMAGE_LAYOUT_GENERAL,
        VK_IMAGE_LAYOUT_GENERAL,
        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    vkCmdCopyImage(
        commandBuffer.commandBuffer,
        originBlockPalette.previous().image,
        VK_IMAGE_LAYOUT_GENERAL,
        originBlockPalette.current().image,
        VK_IMAGE_LAYOUT_GENERAL,
        1,
        &static_palette_copy
    );
...
void LumInternal::LumInternalRenderer::map_mesh(InternalMeshModel* mesh, MeshTransform* mesh_trans) {
    CommandBuffer& commandBuffer = computeCommandBuffers.current();
    VkDescriptorImageInfo modelVoxelsInfo = {};
    modelVoxelsInfo.imageView = (*mesh).voxels.current().view; // CHANGE ME
    modelVoxelsInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
    VkWriteDescriptorSet modelVoxelsWrite = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};
    ...
    struct {
        mat4 trans;
        ivec4 shift;
    } itrans_shift = {mat4(inverse(transform)), ivec4(border.min, 0)};
    ...
    // commandBuffer.cmdDispatch ((map_area.x * 3 + 3) / 4, (map_area.y * 3 + 3) / 4, (map_area.z *
    // 3 + 3) / 4);
...
void LumInternal::LumInternalRenderer::lightmap_start_blocks() {
    ...
    /**/ commandBuffer
        .cmdBindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, lightmapBlocksPipe.lineLayout, 0, 1, &lightmapBlocksPipe.sets.current(), 0, 0);
}
...
void LumInternal::LumInternalRenderer::lightmap_start_models() {
    ...
    /**/ commandBuffer
        .cmdBindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, lightmapModelsPipe.lineLayout, 0, 1, &lightmapModelsPipe.sets.current(), 0, 0);
}
...
void LumInternal::LumInternalRenderer::start_raygen() {
    ...
    struct unicopy {
        ...
        vec4(camera.cameraPos, 0),
        vec4(camera.cameraDir, 0),
        vec4(camera.horizline * camera.viewSize.x / 2.0, 0),
        vec4(camera.vertiline * camera.viewSize.y / 2.0, 0),
        vec4(lightDir, 0),
        mat4(lightTransform),
...
void LumInternal::LumInternalRenderer::raygen_start_blocks() {
    ...
    /**/ commandBuffer
        .cmdBindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, raygenBlocksPipe.lineLayout, 0, 1, &raygenBlocksPipe.sets.current(), 0, 0);
}
...
void LumInternal::LumInternalRenderer::draw_block_face(i8vec3 normal, IndexedVertices& buff, int block_id) {
    ...
    u8 pbn = (sign << 7 | absnorm.x << 0 | absnorm.y << 1 | absnorm.z << 2);

    // signBit_4EmptyBits_xBit_yBit_zBit
    struct {
        u8vec4 inorm;
    } norms = {u8vec4(pbn, 0, 0, 0)};

    vkCmdPushConstants(
        commandBuffer.commandBuffer,
        raygenBlocksPipe.lineLayout,
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
        8,
        sizeof(norms),
        &norms
...
void LumInternal::LumInternalRenderer::raygen_block(InternalMeshModel* block_mesh, int block_id, ivec3 shift) {
    ...
    */
    struct {
        i16 block;
        i16vec3 shift;
    } blockshift = {i16(block_id), i16vec3(shift)};
...
void LumInternal::LumInternalRenderer::raygen_start_models() {
    ...
    /**/ commandBuffer
        .cmdBindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, raygenModelsPipe.lineLayout, 0, 1, &raygenModelsPipe.sets.current(), 0, 0);
}
...
void LumInternal::LumInternalRenderer::raygen_model(InternalMeshModel* model_mesh, MeshTransform* model_trans) {
    ...
    */
    struct {
        quat rot;
        vec4 shift;
    } rotshift = {model_trans->rot, vec4(model_trans->shift, 0)};
...
void LumInternal::LumInternalRenderer::lightmap_block(InternalMeshModel* block_mesh, int block_id, ivec3 shift) {
    ...
    */
    struct {
        i16vec4 shift;
    } blockshift = {i16vec4(shift, 0)};

    vkCmdPushConstants(
        lightmap_commandBuffer.commandBuffer,
        lightmapBlocksPipe.lineLayout,
        VK_SHADER_STAGE_VERTEX_BIT,
        0,
        sizeof(blockshift),
        &blockshift
...
void LumInternal::LumInternalRenderer::raygen_map_particles() {
    ...
    if (!particles.empty()) { // just for safity
        ...
        // raygenParticlesPipe.lineLayout, 0, 1, &raygenParticlesPipe.sets.current(), 0, 0);
...
void LumInternal::LumInternalRenderer::raygen_start_grass() {
    ...
    // /**/ commandBuffer.cmdBindDescriptorSets (VK_PIPELINE_BIND_POINT_GRAPHICS,
    // raygenGrassPipe.lineLayout, 0, 1, &raygenGrassPipe.sets.current(), 0, 0);
}
...
void LumInternal::LumInternalRenderer::updade_grass(vec2 windDirection) {
    ...
    struct {
        vec2 wd, cp;
        float dt;
    } pushconstant = {windDirection, {}, float(lumal.iFrame)};

    vkCmdPushConstants(
        commandBuffer.commandBuffer,
        updateGrassPipe.lineLayout,
        VK_SHADER_STAGE_COMPUTE_BIT,
        0,
        sizeof(pushconstant),
        &pushconstant
    ...
    PLACE_TIMESTAMP_OUTSIDE(commandBuffer.commandBuffer);
    commandBuffer.cmdDispatch((world_size.x * 2 + 7) / 8, (world_size.y * 2 + 7) / 8,
                              1); // 2x8 2x8 1x1
    PLACE_TIMESTAMP_OUTSIDE(commandBuffer.commandBuffer);
}
...
void LumInternal::LumInternalRenderer::updade_water() {
    ...
    struct {
        vec2 wd;
        float dt;
    } pushconstant = {{}, float(lumal.iFrame)};

    vkCmdPushConstants(
        commandBuffer.commandBuffer,
        updateWaterPipe.lineLayout,
        VK_SHADER_STAGE_COMPUTE_BIT,
        0,
        sizeof(pushconstant),
        &pushconstant
...
void LumInternal::LumInternalRenderer::raygen_map_grass(InternalMeshFoliage* grass, ivec3 pos) {
    ...
    struct {
        ...
        int _size, _time;
        int xf, yf;
    } raygen_pushconstant = {vec4(pos, 0), size, lumal.iFrame, x_flip, y_flip};

    vkCmdPushConstants(
        commandBuffer.commandBuffer,
        grass->pipe.lineLayout,
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
        0,
        sizeof(raygen_pushconstant),
        &raygen_pushconstant
    );
    const int verts_per_blade = grass->vertices; // for triangle strip
    const int blade_per_instance = 1; // for triangle strip
    commandBuffer.cmdDraw(verts_per_blade * blade_per_instance, (size * size + (blade_per_instance - 1)) / blade_per_instance, 0, 0);
}
...
void LumInternal::LumInternalRenderer::raygen_start_water() {
    ...
    /**/ commandBuffer
        .cmdBindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, raygenWaterPipe.lineLayout, 0, 1, &raygenWaterPipe.sets.current(), 0, 0);
}
...
void LumInternal::LumInternalRenderer::raygen_map_water(InternalMeshLiquid water, ivec3 pos) {
    ...
    struct {
        vec4 _shift;
        int _size, _time;
    } raygen_pushconstant = {vec4(pos, 0), qualitySize, lumal.iFrame};

    vkCmdPushConstants(
        commandBuffer.commandBuffer,
        raygenWaterPipe.lineLayout,
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
        0,
        sizeof(raygen_pushconstant),
        &raygen_pushconstant
    ...
    commandBuffer.cmdDraw(verts_per_water_tape, tapes_per_block, 0, 0);
}
...
void LumInternal::LumInternalRenderer::start_2nd_spass() {
    CommandBuffer& commandBuffer = graphicsCommandBuffers.current();
    auto ao_lut = generateLUT<8>(
        16.0 / 1000.0,
        dvec2(lumal.swapChainExtent.width, lumal.swapChainExtent.height),
        dvec3(camera.horizline * camera.viewSize.x / 2.0),
        dvec3(camera.vertiline * camera.viewSize.y / 2.0)
    );
    commandBuffer.cmdPipelineBarrier(
        VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
        VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
    ...
    //     renderPassInfo.renderArea.extent = {render.swapChainExtent.width,
    //     render.swapChainExtent.height}; renderPassInfo.clearValueCount =
    //     Lumal::clearColors.size(); renderPassInfo.pLumal::ClearValues =
    //     Lumal::clearColors.data();
...
void LumInternal::LumInternalRenderer::diffuse() {
    ...
    struct rtpc {
        vec4 v1, v2;
        mat4 lp;
    } pushconstant = {vec4(camera.cameraPos, intBitsToFloat(lumal.iFrame)), vec4(camera.cameraDir, 0), lightTransform};

    vkCmdPushConstants(
        commandBuffer.commandBuffer,
        diffusePipe.lineLayout,
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
        0,
        sizeof(pushconstant),
        &pushconstant
...
void LumInternal::LumInternalRenderer::glossy_raygen() {
    ...
    /**/ commandBuffer.cmdBindDescriptorSets(
        VK_PIPELINE_BIND_POINT_GRAPHICS,
        fillStencilGlossyPipe.lineLayout,
        0,
        1,
        &fillStencilGlossyPipe.sets.current(),
        0,
        0
    );
    PLACE_TIMESTAMP_OUTSIDE(commandBuffer.commandBuffer);
...
void LumInternal::LumInternalRenderer::raygen_start_smoke() {
    ...
    /**/ commandBuffer.cmdBindDescriptorSets(
        VK_PIPELINE_BIND_POINT_GRAPHICS,
        fillStencilSmokePipe.lineLayout,
        0,
        1,
        &fillStencilSmokePipe.sets.current(),
        0,
        0
    );
}
...
void LumInternal::LumInternalRenderer::raygen_map_smoke(InternalMeshVolumetric smoke, ivec3 pos) {
    ...
    struct rtpc {
        vec4 centerSize;
    } pushconstant = {vec4(vec3(pos) * 16.f, 32)};
...
void LumInternal::LumInternalRenderer::glossy() {
    ...
    struct rtpc {
        vec4 v1, v2;
    } pushconstant = {vec4(camera.cameraPos, 0), vec4(camera.cameraDir, 0)};

    vkCmdPushConstants(
        commandBuffer.commandBuffer,
        glossyPipe.lineLayout,
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
        0,
        sizeof(pushconstant),
        &pushconstant
...
void LumInternal::LumInternalRenderer::end_frame() {
    TRACE();
    // graphicsCommandBuffers.cmdExplicitTransLayoutBarrier (urrent(), VK_IMAGE_LAYOUT_UNDEFINED,
    // VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
    //     VK_PIPELINE_STAGE_ALL_COMMANDS_BIT | VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
    //     VK_PIPELINE_STAGE_ALL_COMMANDS_BIT | VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
    //     VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT |
    //     VK_ACCESS_MEMORY_WRITE_BIT, &lumal.swapchainImages.current());
    ...
    //     VK_PIPELINE_STAGE_ALL_COMMANDS_BIT | VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
    //     VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT |
    //     VK_ACCESS_MEMORY_WRITE_BIT, &lumal.swapchainImages.previous());
    ...
    //     VK_PIPELINE_STAGE_ALL_COMMANDS_BIT | VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
    //     VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT |
    //     VK_ACCESS_MEMORY_WRITE_BIT, &lumal.swapchainImages.next());
