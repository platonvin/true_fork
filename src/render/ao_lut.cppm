...
// computes lut for ambient occlusion shader
...
vec3 get_world_shift_from_clip_shift(vec2 clip_shift, vec3 horizline_scaled, vec3 vertiline_scaled) {
    vec3 shift = (horizline_scaled * clip_shift.x) + (vertiline_scaled * clip_shift.y);
    return shift;
}

double calculate_total_weight(int sample_count, double max_radius, dvec2 frame_size, dvec3 horizline_scaled, dvec3 vertiline_scaled) {
...
export template<size_t sample_count>
std::array<AoLut, sample_count> generateLUT(double max_radius, dvec2 frame_size, dvec3 horizline_scaled, dvec3 vertiline_scaled) {
